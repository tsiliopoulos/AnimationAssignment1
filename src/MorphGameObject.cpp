...
void MorphGameObject::update(float dt)
{
	...
	m_pLocalMorphTime += dt * m_pMorphTargets[m_pCurrentKeyframe]->SpeedMultiplier;
...
void MorphGameObject::setSpeedMultiplier(std::shared_ptr<TTK::MeshBase> CurrentKey, std::shared_ptr<TTK::MeshBase> NextKey, float Multiplier) {
	CurrentKey->SpeedMultiplier = Multiplier / (glm::distance(CurrentKey->vertices[0], NextKey->vertices[0]));
}
